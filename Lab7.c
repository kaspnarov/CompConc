...
/* Aluno: Thiago Duarte Campbell de Medeiros (118060285) */
...
void *t1 (void *threadid) {
    sem_wait(&condt1); //espera T2 e T3 executarem
    printf("Volte sempre!\n");
    pthread_exit(NULL);
}
...
void *t2 (void *threadid) {
    sem_wait(&condt2); //espera T4 executar
    printf("Fique a vontade.\n");
    sem_post(&condt2); //permite que T3 execute
    sem_post(&condt3); //permite que T3 libere a T1
    pthread_exit(NULL);
}
...
void *t4 (void *threadid) {
    ...
    sem_post(&condt2); //permite que T2 ou T3 executem
    pthread_exit(NULL);
}
